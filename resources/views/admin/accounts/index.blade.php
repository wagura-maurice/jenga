...
<div id="content" class="content">
    ...
    <div class="row">
        <div class="col-md-12">
            @if($accountsdata['usersaccountsroles'][0]->_add==1)
            <a href="{!! route('accounts.create') !!}"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-plus"></i></button></a>
            @endif
            @if($accountsdata['usersaccountsroles'][0]->_report==1)
            <a href="{!! url('admin/accountsreport') !!}"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-file-text-o"></i></button></a>
            @endif
            @if($accountsdata['usersaccountsroles'][0]->_report==1 || $accountsdata['usersaccountsroles'][0]->_list==1)
            <a href="#modal-dialog"  data-toggle="modal"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-filter"></i></button></a>
            @endif
<!--             @if($accountsdata['usersaccountsroles'][0]->_add==1)
            <a href="{!! url('/admin/accountsdbimport') !!}"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-download"></i></button></a>
            @endif -->            
        ...
        <div class="col-md-12">
            <div class="panel panel-inverse">
                ...
                <form action="{!! route('accounts.destroy',$accounts->id) !!}" method="POST">
                                        ...
                                        <a href="{!! route('accounts.show',$accounts->id) !!}" id='show-accounts-{!! $accounts->id !!}' class='btn btn-sm btn-circle btn-inverse'>
                                            <i class='fa fa-btn fa-eye'></i>
                                        </a>
                                        @endif
                                        @if($accountsdata['usersaccountsroles'][0]->_edit==1)
                                        <a href="{!! route('accounts.edit',$accounts->id) !!}" id='edit-accounts-{!! $accounts->id !!}' class='btn btn-sm btn-circle btn-warning'>
                                        ...
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                                        <button type='submit' id='delete-accounts-{!! $accounts->id !!}' class='btn btn-sm btn-circle btn-danger'>
