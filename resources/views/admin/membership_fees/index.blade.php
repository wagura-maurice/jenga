...
<div id="content" class="content">
    ...
    <h1 class="page-header">Membership Fees - DATA <small>membership fees data goes here...</small></h1>
    <div class="row">
        <div class="col-md-12">
            @if($membershipfeesdata['usersaccountsroles'][0]->_add==1)
            <a href="{!! route('membershipfees.create') !!}"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-plus"></i></button></a>
            @endif
            @if($membershipfeesdata['usersaccountsroles'][0]->_report==1)
            <a href="{!! url('admin/membershipfeesreport') !!}"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-file-text-o"></i></button></a>
            @endif
            @if($membershipfeesdata['usersaccountsroles'][0]->_report==1 || $membershipfeesdata['usersaccountsroles'][0]->_list==1)
            <a href="#modal-dialog"  data-toggle="modal"><button type="button" class="btn btn-inverse btn-icon btn-circle m-b-10"><i class="fa fa-filter"></i></button></a>
            @endif
    @if($membershipfeesdata['usersaccountsroles'][0]->_list==1)
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
                    ...
                    <table id="data-table" class="table table-striped table-bordered">
                        ...
                        @foreach ($membershipfeesdata['list'] as $membershipfees)
                            <tr>
                                ...
                                <td>
                <form action="{!! route('membershipfees.destroy',$membershipfees->id) !!}" method="POST">
                                        @if($membershipfeesdata['usersaccountsroles'][0]->_show==1)
                                        <a href="{!! route('membershipfees.show',$membershipfees->id) !!}" id='show-membershipfees-{!! $membershipfees->id !!}' class='btn btn-sm btn-circle btn-inverse'>
                                            <i class='fa fa-btn fa-eye'></i>
                                        </a>
                                        @endif
  
                                        @if($membershipfeesdata['usersaccountsroles'][0]->_delete==1)
                                        <input type="hidden" name="_method" value="delete" />
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                                        <button type='submit' id='delete-membershipfees-{!! $membershipfees->id !!}' class='btn btn-sm btn-circle btn-danger'>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Membership Fees - Filter Dialog</h4>
            </div>
    			...
    			<div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-inverse">
                            ...
                            <div class="panel-body">
                    <form id="form"  class="form-horizontal" action="{!! url('admin/membershipfeesfilter') !!}" method="POST">
                        {!! csrf_field() !!}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-offset-3 col-sm-6">
                                        <button type="submit" class="btn btn-sm btn-inverse">
                                            <i class="fa fa-btn fa-search"></i> Search Membership Fees
                                        </button>
                                    </div>
                                </div>
                    </form>
