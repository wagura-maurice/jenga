...
<div id="content" class="content">
    ...
    <h1 class="page-header">Membership Fees Form <small>membership fees details goes here...</small></h1>
    <div class="row">
        <div class="col-md-12">
                    ...
                    <form id="form"  class="form-horizontal" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
									    <select class="form-control selectpicker" data-size="100000" data-live-search="true" data-style="btn-white" name="client"  id="client" onchange="membershipFee()">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-3 col-sm-6">
                                        <button type="button" onclick="save()" class="btn btn-sm btn-primary">
                                            <i class="fa fa-btn fa-plus"></i> Add Membership Fees
                                        </button>
                                    </div>
                                </div>
                    </form>
...
function membershipFee(){
    ...
    $.ajax({
        type:'POST',
        url: "{!! url('/admin/getclienttypemembershipfees/')!!}",
        data:JSON.stringify({'client':client,'_token':"{{ csrf_token() }}"}),
...
function save(){
    if(Number($("#_amount").val())!=Number($("#amount").val())){
        alert("Membership fee must be "+$("#_amount").val());
        return;
    }
	var formData = new FormData($('#form')[0]);
	$.ajax({
		type:'POST',
		url: "{!! route('membershipfees.store')!!}",
		data:formData,
		cache:false,
		contentType: false,
		processData: false,
