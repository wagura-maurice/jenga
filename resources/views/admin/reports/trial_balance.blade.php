...
@section('main_content')
    <div id='content' class='content'>
        ...
        <h1 class="page-header hidden-print">Trial Balance <small>Trial Balance report goes here...</small></h1>
        <div class="row">
            <form>
        ...
        <div class="row no-print">
            <div class="col-md-12 hidden-print">
                <a href="{{ route('generalledgers.index') }}"><button type="button"
            ...
            <div class="invoice-content">
                        ...
                        <tbody>
                            @foreach ($trialbalancedata['ledger'] as $ledger)
                                <tr>
                                    ...
                                    <td class='table-text'>
                                        <div>{{ $ledger['debit'] }}</div>
                                    </td>
                                    <td class='table-text'>
                                        <div>{{ $ledger['credit'] }}</div>
                                    </td>
                                </tr>
                            @endforeach
                            <tr style="font-weight: bold;">
                                ...
                                <td class='table-text'>
                                    <div>{{ $trialbalancedata['totaldebit'] }}</div>
                                </td>
                                <td class='table-text'>
                                    <div>{{ $trialbalancedata['totalcredit'] }}</div>
                                </td>
                            </tr>
                            <tr style="font-weight: bold;">
                                <td class='table-text'>
                                ...
                                <td class='table-text'>
                                    <div><button type="button" onclick="paginate()"
                                            class="btn btn-inverse">Previous</button></div>
                                </td>
                                <td class='table-text'>
                                    <div><button type="button" onclick="paginate()" class="btn btn-inverse">Next</button>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="invoice-price">
                    <div class="invoice-footer text-muted">
                        ...
                        <p class="text-center">
                            <span class="m-r-10"><i class="fa fa-globe"></i>
                                {{ $trialbalancedata['company'][0]->website }}</span>
                            <span class="m-r-10"><i class="fa fa-phone"></i>
                                T:{{ $trialbalancedata['company'][0]->phone_number }}</span>
                            <span class="m-r-10"><i class="fa fa-envelope"></i>
                                {{ $trialbalancedata['company'][0]->email_address }}</span>
                        </p>
                    </div>
                </div>
            </div>
        ...
        <script>
            function paginate() {
                ...
                window.location.href = "{{ url('/admin/trialbalance/') }}/" + pageIndex + "/" + pageSize;
