                    ...
                    <form id="form"  class="form-horizontal" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
									    <select class="form-control selectpicker" data-size="100000" data-live-search="true" data-style="btn-white" name="loan_number"  id="loan_number" onchange="getloaninsuarancedeductionfee()">
                                            <option value="">Select Loan</option>
                                            @foreach ($insurancedeductionpaymentsdata['loans'] as $loans)
                                                @if ($loans->insurance_deduction_fee != 0)
    				                                <option value="{{ $loans->id }}" {{ ($loans->insurance_deduction_fee - \App\InsuranceDeductionPayments::getPaymentsSum($loans->id)) == 0 ? 'disabled' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
									    <select class="form-control selectpicker" data-size="100000" data-live-search="true" data-style="btn-white" name="payment_mode"  id="payment_mode">
                                            ...
                                            @foreach ($insurancedeductionpaymentsdata['paymentmodes'] as $paymentmodes)
				                                <option value="{!! $paymentmodes->id !!}">{!! $paymentmodes->name!!}</option>
			                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
									    <select class="form-control selectpicker" data-size="100000" data-live-search="true" data-style="btn-white" name="transaction_status"  id="transaction_status">
                                            <option disabled>Select Transaction Status</option>				                                @foreach ($insurancedeductionpaymentsdata['transactionstatuses'] as $transactionstatuses)
				                                <option value="{!! $transactionstatuses->id !!}">{!! $transactionstatuses->name!!}</option>
			                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="col-sm-offset-3 col-sm-6">
                                        <button type="button" onclick="save()" class="btn btn-sm btn-primary" id="btn-save">
                                            <i class="fa fa-btn fa-plus"></i> Add Insurance Deduction Payments
                                        </button>
                                    </div>
                                </div>
                    </form>
...
function getloaninsuarancedeductionfee(){
    var loan_number=$("#loan_number").val();
    // var token=$("input[name=_token]").val();
    $.ajax({
        type:'POST',
        url: "{!! url('/admin/getloaninsuarancedeductionfee/')!!}",
...
function save(){
	...
	$.ajax({
		type:'POST',
		url: "{!! route('insurancedeductionpayments.store')!!}",
		data:formData,
		cache:false,
		contentType: false,
		processData: false,
